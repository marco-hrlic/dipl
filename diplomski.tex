...
\renewcommand{\min}{\text{\normalfont{min\ }}}

\newenvironment{alg}[1]
{
    \bigskip
    \begin{tcolorbox}[arc=0mm,boxrule=1pt,colframe=black,colback=white,detach title, before upper={\begin{center}{\bigskip\textbf{#1}\bigskip\hline\medskip}\end{center}},frame hidden]
}
{
    \medskip
\end{tcolorbox}
...
\end{equation}
To je zapravo konveksna relaksacija problema \eqref{problem_minimizacije} i zovemo ga $\ell_1$\textit{-minimizacija}.
...
\end{thm}
\begin{proof}
    Pretpostavimo suprotno, tj. da je skup $\{\vec a_j,\ j \in \supp(\vec x^{\sharp})\}$ linearno zavisan. Neka je $S= \supp(\vec x^{\sharp})$. To zna\v{c}i da postoji ne-nul vektor $\vec v \in \R^N$ sa nosa\v{c}em $S$.
\end{proof}
