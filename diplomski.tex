...
\chapter[Rjetka rje\v{s}enja][Rjetka rje\v{s}enja]{Rjetka rje\v{s}enja}	
% ukoliko naslov nije jako dugacak dovoljno je samo \chapter{Naslov poglavlja} 

\section[Naslov sekcije u sadržaju][Kratki naslov sekcije]{Sparsity and compressibilty}
...
\begin{defn}
	...
	$$supp(x):=\{j\in[N]:x_j \neq 0 \}$$
\end{defn}
Za vektor $x\in\C^{\N}$ ka\v{z}emo da je $s$-rjedak ako vrijedi
$\|x\|_0 := card(supp(x)) \leq s$.




\begin{thm} Iskaz teorema u kojem se javljaju skupovi  $\N$, $\Z$, $\Q$, $\R$ i $\C$.
\end{thm}
\begin{conj}
